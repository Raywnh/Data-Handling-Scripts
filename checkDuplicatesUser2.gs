/*
 * Function: checkDuplicatesUser2() - checks the sheet for whether there is a duplicate data entry based on user's student/staff ID and deletes the older data entry
 * Parameter(s): none
 */
...
function checkDuplicatesStaff() {
  ...
  var user2Sheet = spreadSheet.getSheetByName('users2!');  
  var sheetName = 'users2!';  
  
  var range = spreadSheet.getRangeByName('sortUser2');  
    ...
    for (let j = i + 1; j < maxLength; j++) {
      ...
      if (duplicateData == currentData) {   // If we get matching ID numbers, then we compare the timestamps of both data entries and archive the older data entry
        ...
        if (duplicateTime < currentTime)  
          moveToOld(j+2,user2sheet, sheetName);
        else  
          moveToOld(i+2,user2sheet, sheetName);
      } 
    }
  }
}
